...
你不需要先懂程式。這條路徑要帶你完成一件具體的事：從空資料夾建立自己的網站，加入頁面與樣式，再用瀏覽器看到它。

第一篇只做工具準備。完成時，你會有一個能輸入指令的視窗、一個編輯檔案的工具，以及兩個成功顯示的版本號。頁首的閱讀時間不包含下載、安裝與練習時間。

## 先看全貌：你、工具與網站的關係

安裝前，先分清楚三種動作：**寫檔案、執行工具、看結果**。同一台電腦會開幾個視窗，是因為它們各自負責不同的工作。

<figure class="learning-diagram">
<figcaption>圖 1：從你寫的檔案，到瀏覽器看到的畫面</figcaption>
<ol>
<li><strong>1. 你與 VS Code</strong><span>在編輯器輸入內容，按 Ctrl+S 儲存。</span></li>
<li><strong>2. 專案資料夾</strong><span>保存你寫的 .astro 頁面與工具清單。</span></li>
<li><strong>3. Astro（由 Node.js 執行）</strong><span>開發模式讀取檔案，處理成瀏覽器可用的內容。</span></li>
<li><strong>4. 瀏覽器</strong><span>開啟 localhost，顯示標題、段落與樣式。</span></li>
</ol>
<p>箭頭表示內容的流向。這整條流程都在你的電腦；尚未發布到網路。</p>
</figure>

PowerShell 是你**下達工作指令**的地方，不是寫文章的地方。pnpm 幫你安裝工具、啟動指定工作；Node.js 則是執行 Astro 的環境。它們並不是四個不同的網站。

| 你要做的事 | 使用哪裡 | 會改變什麼 |
| --- | --- | --- |
| 寫標題、段落或樣式 | VS Code 編輯區 | 儲存後改變原始檔 |
| 安裝 Astro、啟動網站 | PowerShell 終端機 | 安裝工具或啟動本機程序 |
| 看成果、點連結 | 瀏覽器 | 顯示頁面，不會替你儲存原始檔 |

接下來的版本指令是在**確認工具找得到**。看到版本號，不代表網站已建立；第二篇才會建立網站檔案。先知道每個步驟的目的，就不用把成功訊息當成需要背下來的咒語。
...
我們使用的 **Astro** 是製作網站的工具。它把你寫的頁面與內容整理成瀏覽器能讀取的檔案。我們會從空資料夾開始，一步一步親手建立自己的檔案。

**GitHub** 是放置與分享程式檔案的網站；一組專案檔案放在一個 **repository（儲存庫）** 中。**Git** 則是記錄檔案修改歷史的工具，與 GitHub 不是同一件事。這次先在電腦建立自己的檔案，所以可以先不安裝 Git。
...
從 [VS Code 官方 Windows 安裝說明](https://code.visualstudio.com/docs/setup/windows) 前往下載 **User Installer**，執行安裝檔並完成安裝，然後開啟 VS Code。這次不用登入帳號、購買服務或安裝 AI 功能。下一篇才會用它開啟自己新建的資料夾。
...
執行完版本檢查後，重新輸入一次 `node --version`。你會再次得到版本號，但沒有因此新增網頁。這是查詢工具資訊，不是寫網站。

下一篇建立頁面時，標題文字會寫進 VS Code 的檔案編輯區。把兩件事分開記：**指令要求電腦做工作；原始檔描述網站要有什麼。**
...
你現在已知道在哪裡輸入指令。下一篇會建立空資料夾與第一個頁面，讓瀏覽器顯示自己寫的網站。使用下方學習路徑的「下一篇」繼續。
